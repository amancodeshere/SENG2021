...
export function getUserBySessionId(sessionId, callback) {
    ...
    db.query(`SELECT UserID FROM sessions WHERE SessionID = $1 AND VALID = true;`, [sessionId])
        .then((res1) => {
            if (res1.rows.length === 0) return callback(new CustomInputError("Session not found or invalid."));
            const userId = res1.rows[0].userid;
...
/**
 * Invalidates a session by setting VALID to false.
 */
export function invalidateSession(sessionId, callback) {
    ...
    db.query(`UPDATE sessions SET VALID = false WHERE SessionID = $1 RETURNING *;`, [sessionId])
        .then(res => {
            if (res.rows.length === 0) return callback(new CustomInputError("Session not found."));
            callback(null, { success: true, message: "Session invalidated successfully." });
        })
        .catch(err => {
